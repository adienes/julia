...
function replace(A, old_new::Pair...; count::Union{Integer,Nothing}=nothing)
    ...
    if isnothing(count)
        T = _pair_eltype(subtract_singletontype(eltype(A), old_new...), V)
        replace_pairs!(_similar_or_copy(A, T), A, typemax(Int), old_new)
    else
        U = _pair_eltype(eltype(A), V)
...
_pair_eltype(T::Type, S::Type) = promote_type(T, S)
function _pair_eltype(::Type{Pair{K1,V1}}, ::Type{Pair{K2,V2}}) where {K1,V1,K2,V2}
