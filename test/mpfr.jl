...
@testset "issue #9816" begin
    ...
    @test ccall((:mpfr_get_emin, MPFR.libmpfr), Clong, ()) == MPFR.get_emin_min()
    @test ccall((:mpfr_get_emax, MPFR.libmpfr), Clong, ()) == MPFR.get_emax_max()
end
...
# PR #54284
import Base.MPFR: clear_flags, had_range_exception

@testset "MPFR flags" begin
    let x, a = floatmin(BigFloat), b = floatmax(BigFloat)
        clear_flags()
        @test !had_range_exception()
        x = 1 / a # overflow, no range exception
        @test !had_range_exception()
        clear_flags()
