...
// The flags that are baked into the inference results stored in the heap. The
// remaining flags only describe native code, which is regenerated by this
// process' JIT whenever a cache is loaded without its native image. Keep this in
// sync with the jl_options fields that Compiler reads: `inlining_enabled` is the
// only one whose value survives into a cache, since `instrumentation_enabled`
// refuses to alter caches and `inbounds_option` only makes concrete evaluation
// more conservative.
#define JI_INFERENCE_FLAGS ((uint8_t)(1u << INLINE_LEVEL))
...
JL_DLLEXPORT uint8_t jl_cache_flags(void)
{
    ...
    flags |= (jl_options.can_inline & 1) << INLINE_LEVEL; // 5-bit
...
JL_DLLEXPORT uint8_t jl_match_cache_flags(uint8_t requested_flags, uint8_t actual_flags)
{
    ...
    if (!supports_pkgimage) {
        // `--pkgimages=existing` also clears the pkgimage bit, but still loads the
        // native image of a cache that has one. It is not representable in a
        // requested configuration, so it is only meaningful for the current process.
        int loads_native = is_pkgimage &&
            jl_options.use_pkgimages == JL_OPTIONS_USE_PKGIMAGES_EXISTING;
        if (!loads_native) {
            // Only the heap is restored, and it is JIT-compiled by this process,
            // which decides optimization level, debug level and bounds checking for
            // itself. Only the flags recorded in the heap have to match.
            return (actual_flags & JI_INFERENCE_FLAGS) == (requested_flags & JI_INFERENCE_FLAGS);
        }
        // Ignore the pkgimage bit (it will be unset in requested_flags)
        actual_flags &= ~1;
    }
...
// The heap payload is compressed as a zstd frame.
static const uint32_t JI_FLAG_COMPRESSED_ZSTD = 1 << 2;
...
static int jl_read_verify_header_impl(ios_t *s, uint32_t *flags, uint32_t *checksum,
                                      int64_t *dataendpos, int64_t *datastartpos,
                                      int64_t *flagspos) JL_CANSAFEPOINT
{
    uint16_t bom;
...
JL_DLLEXPORT int jl_read_verify_header(ios_t *s, uint32_t *flags, uint32_t *checksum,
                                       int64_t *dataendpos, int64_t *datastartpos) JL_CANSAFEPOINT
{
    return jl_read_verify_header_impl(s, flags, checksum, dataendpos, datastartpos, NULL);
}
